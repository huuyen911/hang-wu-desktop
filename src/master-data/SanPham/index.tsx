...
function validate(
  form: SanPhamFormValues,
): Partial<Record<keyof SanPhamFormValues, string>> {
...
export default function SanPhamPage() {
  ...
  const displayFiltered = useMemo(() => {
    ...
    if (filterBrand)
      result = result.filter((i) => i.thuong_hieu === filterBrand);
    if (filterChinhElva)
      result = result.filter((i) => i.la_san_pham_chinh_elvawell);
    if (filterChinhWei)
      result = result.filter((i) => i.la_san_pham_chinh_weilaiya);
    return result;
  ...
  const isAnyFilterActive =
    !!c.search || !!filterBrand || filterChinhElva || filterChinhWei;
    ...
    >
      ...
      >
        ...
        <NumberInput
          ...
          onChange={(val) =>
            setForm((f) => ({ ...f, quy_cach: val as number | "" }))
          }
        ...
        <Select
          ...
          onChange={(val) =>
            setForm((f) => ({ ...f, thuong_hieu: (val as ThuongHieu) ?? "" }))
          }
        ...
        <Switch
          ...
          onChange={(e) => {
            ...
            setForm((f) => ({ ...f, la_san_pham_chinh_elvawell: v }));
          }}
          mt={4}
        />
        <Switch
          ...
          onChange={(e) => {
            const v = e.currentTarget.checked;
            setForm((f) => ({ ...f, la_san_pham_chinh_weilaiya: v }));
          }}
          mt={4}
        />
      </FormModal>
