...
export const THUONG_HIEU_OPTIONS = THUONG_HIEU_VALUES.map((v) => ({
  value: v,
  label: v,
}));

export const NHAN_VIEN_OPTIONS = NHAN_VIEN_VALUES.map((v) => ({
  value: v,
  label: v,
}));
...
const BRAND_CONFIG: Record<string, BrandConfig> = {
  Elvawell: {
    ...
    ceoPrefix: "E",
    color: "violet",
    mainField: "la_san_pham_chinh_elvawell",
  },
  Weilaiya: {
    short: "W",
    ceoPrefix: "W",
    color: "blue",
    mainField: "la_san_pham_chinh_weilaiya",
  },
};
...
export const ALL_BRAND_PREFIXES: readonly string[] = Object.values(
  BRAND_CONFIG,
).map((c) => c.ceoPrefix);
