...
import { Stack, Title, Text, Alert, Button, Center, Loader } from '@mantine/core'
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (rows.length === 0) {
        setLocal({
          status: 'error',
          message: 'Không tìm thấy dữ liệu hợp lệ trong file. Hãy kiểm tra lại định dạng.',
        })
        return
      }
    ...
    <Stack h="100%" gap={0} style={{ overflow: 'auto' }}>
      ...
      >
        ...
        <Text size="sm" c="dimmed">
          Tổng hợp dữ liệu từ file Excel — xử lý hoàn toàn trên máy, không gửi dữ liệu ra ngoài
        </Text>
      </Stack>
      ...
      {local.status === 'error' && (
        <Center flex={1}>
          <Stack align="center" gap="md" maw={420} w="100%">
            ...
            <Button variant="light" color="red" onClick={() => setLocal({ status: 'idle' })}>
              Thử lại
            </Button>
          </Stack>
        </Center>
