...
export default function FileDropZone({ onPath, large = false }: Props) {
  ...
  const iconColor = dragOver ? 'var(--mantine-color-blue-6)' : 'var(--mantine-color-gray-5)'
      ...
      >
        {large ? (
          <Stack align="center" gap="sm" py="xl" px="md">
            ...
            <Button variant="light" size="sm" mt="xs" onClick={(e) => { e.stopPropagation(); pickFile() }}>
              Chọn file
            </Button>
          </Stack>
        ) : (
          <Group justify="space-between" align="center" px="md" py="sm">
            ...
            <Button variant="light" size="xs" onClick={(e) => { e.stopPropagation(); pickFile() }}>
              Chọn file
            </Button>
          </Group>
        )}
