...
let pendingSnapshot: { qc: QueryClient; id: number; rows: SalesRow[] } | null =
  null;
...
export function flushSavePending(): Promise<void> {
  ...
  inFlight = inFlight
    .catch(() => undefined)
    .then(() => runSave(snap.qc, snap.id, snap.rows));
  return inFlight;
}
...
export function useSalesRows() {
  ...
  const { data: session = null, isLoading } = useQuery<SalesSession | null>({
    queryKey:
      activeId != null
        ? sessionDetailKey(activeId)
        : (["sales-sessions", "detail", null] as const),
  ...
  const setActiveId = useCallback(
    (id: number | null) => setActiveIdStore(id),
    [],
  );
