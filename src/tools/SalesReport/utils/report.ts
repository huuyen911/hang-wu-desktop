...
export function mergeCEOsAcrossBrands(
  ...
  allBrands.forEach((b) => {
    b.ceos.forEach((ceo) => {
      ...
      ceo.months.forEach((m) => {
        if (!entry.monthsMap.has(m.month))
          entry.monthsMap.set(m.month, new Map());
  ...
  const merged: CEOSummary[] = [...ceoMergedMap.entries()].map(
    ([ceoCode, { ceoName, monthsMap }]) => {
