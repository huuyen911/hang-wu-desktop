    ...
    <Group gap={8} wrap="nowrap" align="center">
      ...
      <Stack gap={0}>
        ...
        <Text
          size="sm"
          fw={700}
          c={accentColor ? undefined : "dark"}
          style={{ lineHeight: 1.3, color: accentColor }}
        >
          {value}
        </Text>
      </Stack>
...
export default function BrandPanel({
  ...
  const ceoOptions = useMemo(
    () =>
      allCEOs
        .filter(
          (c) =>
            masterCEOCodeSetForBrand.has(c.ceo) || !masterCEOCodeSet.has(c.ceo),
        )
        .map((c) => ({
          value: c.ceo,
          label: c.ceoName ? `${c.ceo} — ${c.ceoName}` : c.ceo,
        })),
      ...
      allCEOs
        .filter((c) => {
            ...
            !c.months.some((m) =>
              m.products.some((p) => {
                ...
                return (
                  ...
                  (groupId !== undefined &&
                    (groupIdToNhomName.get(groupId) ?? "").includes(search))
                );
              }),
            )
          )
          ...
          if (
            ...
            !c.months.some((m) =>
              m.products.some((p) =>
                p.invoiceCodes.some((inv) =>
                  normalizeSearch(inv).includes(qInvoice),
                ),
              ),
            )
          )
  ...
  const totalAmountFiltered = visibleCEOs.reduce(
    (s, c) => s + c.totalAmount,
    0,
  );
  ...
  const activeTotalThung = activeCEO
    ? calcTotalThung(
        activeCEO,
        brand.brand,
        quyCachMap,
    ...
    : 0;
  const activeInMasterData = activeCEO
    ? masterCEOCodeSet.has(activeCEO.ceo)
    : true;
    ...
    <Stack h="100%" gap={0}>
      ...
      >
        <Select
          ...
          filter={({ options, search: s }) =>
            options.filter((opt) =>
              "group" in opt
                ? true
                : normalizeSearch(opt.label).includes(normalizeSearch(s)),
            )
          }
        />
        <SearchInput
        ...
        <Select
          ...
          onChange={(v) =>
            onFiltersChange({
              importStatus: v as "imported" | "not-imported" | null,
            })
          }
          data={[
                      ...
                      const crossBrands = new Set(
                        c.months
                          .flatMap((m) =>
                            m.products.map(
                              (p) =>
                                productBrandMap.get(p.productCode) ??
                                brand.brand,
                            ),
                          )
                  ...
                  <Group gap="sm" align="center">
                    ...
                    <div>
                      <Group gap="xs">
                        <Text
                          fw={700}
                          c={activeInMasterData ? undefined : "red.6"}
                        >
                          {activeCEO.ceo}
                        </Text>
                        {activeCEO.ceoName && <Text c="dimmed">·</Text>}
                        {activeCEO.ceoName && (
                          <Text fw={500}>{activeCEO.ceoName}</Text>
                        )}
                      </Group>
                      <Group gap="md" mt={2}>
                        ...
                        <Text size="xs" c="green.7" fw={600}>
                          {fmtAmount(activeCEO.totalAmount)}
                        </Text>
                      </Group>
                    </div>
                  </Group>
