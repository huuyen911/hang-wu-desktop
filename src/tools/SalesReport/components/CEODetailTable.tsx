...
import { Badge, Box, Center, Group, Stack, Table, Text } from "@mantine/core";
  ...
  return (
    <Stack gap="lg">
      {months.map((m) => {
        ...
        const groupThungsArr = orderedGroups.map(
          ({ nhom, products: groupProds }) => {
            if (nhom === null) {
              // Chưa phân nhóm: floor từng mã sản phẩm, rồi cộng
              return groupProds.reduce((s, p) => {
                const productBrand =
                  productBrandMap.get(p.productCode) ?? brand;
            ...
            return Math.floor(
              groupProds.reduce((s, p) => {
                ...
                return (
                  s +
                  (p.rawLines ?? []).reduce((s2, line) => s2 + line.qty / qc, 0)
                );
              }, 0),
            );
          },
          ...
          <Box key={m.month}>
            ...
            >
              ...
              <Text size="xs" c="dimmed">
                {m.products.length} sản phẩm
              </Text>
              <Text size="xs" c="dimmed">
                ·
              </Text>
              <Text size="xs" fw={600} c="dark">
                Số lượng: {fmtQty(m.totalQty)}
              </Text>
              {roundedMonthThung > 0 && (
                <>
                  <Text size="xs" c="dimmed">
                  ...
                  <Text size="xs" fw={600} c="dark">
                    Số lượng thùng: {fmt.format(roundedMonthThung)}
                  </Text>
                </>
              )}
              <Text
            ...
            <Table fz="xs" style={{ tableLayout: "fixed" }}>
              <Table.Thead>
                <Table.Tr>
                  ...
                  <Table.Th style={{ width: "7%", textAlign: "right" }}>
                    Số lượng
                  </Table.Th>
                  ...
                  <Table.Th style={{ width: "13%", textAlign: "right" }}>
                    Thành tiền
                  </Table.Th>
                </Table.Tr>
              </Table.Thead>
                      ...
                      <Fragment key={nhom?.id ?? "ungrouped"}>
                        ...
                        <Table.Tr key={`group-${nhom?.id ?? "ungrouped"}`}>
                          ...
                          >
                            <Group gap="xs">
                              {isCrossBrand && (
                                <BrandTag brand={nhom.thuong_hieu} />
                              )}
                              <Text
                                size="xs"
                                fw={700}
                                c={isUngrouped ? "red.7" : "teal.8"}
                              >
                              ...
                              {roundedGroupThung > 0 && (
                                <>
                                  ...
                                  <Text
                                    size="xs"
                                    fw={600}
                                    c={isUngrouped ? "red.6" : "teal.7"}
                                  >
                                    Số lượng thùng:{" "}
                                    {fmt.format(roundedGroupThung)}
                                  </Text>
                                </>
                              )}
                            </Group>
                        ...
                        {groupProds.flatMap((p) => {
                          ...
                          return rawLines.map((line, lineIdx) => {
                            ...
                            return (
                              <Table.Tr
                                key={`${m.month}-${p.productCode}-${lineIdx}`}
