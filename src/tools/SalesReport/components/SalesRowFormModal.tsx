...
export default function SalesRowFormModal({ opened, row, onClose, onSubmit }: Props) {
  const isEdit = !!row
  ...
  function handleCeoSelect(maCeo: string | null) {
    if (!maCeo) { setD((prev) => ({ ...prev, ceo: '', ceoName: '' })); return }
  ...
  function handleBrandChange(v: string) {
    setD((prev) => ({ ...prev, brand: v, productCode: '', productName: '', unit: '' }))
  }

  function handleProductSelect(maSp: string | null) {
    if (!maSp) { setD((prev) => ({ ...prev, productCode: '', productName: '', unit: '' })); return }
  ...
  return (
    <Modal
      opened={opened}
      onClose={onClose}
      title={<Text fw={600}>{isEdit ? 'Chỉnh sửa dòng' : 'Thêm dòng mới'}</Text>}
      size="lg"
    >
      <Stack gap="sm">
